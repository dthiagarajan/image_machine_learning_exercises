...
def train_action(model, current_epoch, ilr, criterion,
    ...
    # only select parameters with gradients on
    params_with_grad = list(requires_grad_filter(all_params))

    print("total num of params: " + str(len(all_params)))
    print("total num of params with grad: " + str(len(params_with_grad)))

    # setup SGD algorithm
    optimizer = optim.SGD(params_with_grad, lr=current_learning_rate,
